...
def get_con():
    conn = pymysql.connect(host='remotemysql.com', port=3306, user='p4CTPjp003', passwd='w91jx7pC6L',
...
# Insert to table with datetime variable
    cursor.execute(f"INSERT into {db_table}.usersExtra VALUES (%s , %s , now())", (user_id, user_name))
    close_conn()
